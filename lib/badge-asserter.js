function BadgeAsserter () {
  ...
  this.assertBadge = function (game_name, oidc_userinfo, badge_name) {
    ...
    var request = $.ajax({
      async: false,
      url: this.gleaner_frontend_host + "collect/badges",
      dataType: "json",
      headers: {
    ...
    if($.inArray(this.badge_total_name, request.responseJSON.earnedBadges) > -1) {
      // Record for this bagde and user does already exist
      // -> User has already earned the badge, abort assertion process
      console.log("Record found for this badge. Abort assertion!");
      return;
    }
    else {
    ...
    // issue the Badge
    this.issueBadge([this.assertion_host_path], this.user_info.email, this.badge_total_name, this.file_path)
  };

  this.issueBadge = function (assertion_host_path, email, badge_total_name, file_path) {
    // Issue the Badge
    OpenBadges.issue(assertion_host_path, function(errors, successes) {
      console.log(errors);
      console.log(successes);

      // TODO MARKO i think there is a bug here request is fired even though only errors
      // if there was a success, add new badge to earnedBadges via Gleaner API
      if(typeof successes !== 'undefined' && successes.length > 0) {
        var request = $.ajax({
          type: "POST",
          url: "http://localhost:3000/collect/badges", // TODO MARKO use variable
          headers: {
              'Email': email,
              'badge_path': badge_total_name
          },
          success: function() {
